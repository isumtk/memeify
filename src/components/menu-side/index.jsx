...
const MenuSide = () => {
	...
	return (
		<aside className="relative flex h-full items-center rounded-s">
			<div className="flex h-full w-14 flex-col items-center gap-2 bg-white pt-1">
				{menuButtons.map((button) => (
					<button
						...
						className={`flex aspect-square w-12 items-center justify-center rounded border-transparent transition ${
							menuState === button.name && 'bg-indigo-600'
						}`}
						key={button.name}
					>
						<ion-icon
							...
							style={{
								...
								color: menuState === button.name && 'rgb(255,255,255)',
							}}
						></ion-icon>
					</button>
				))}
			</div>
			<div className="h-full w-80 bg-slate-200 transition">
